...
$(document).ready(function() {
    ...
    if (view == "today") {
        ...
        $('.tomorrow').hide();
        $('.today').show();
        $('.today').removeClass('col-xs-3').addClass('col-xs-12');
        // $('.today').addClass('col-xs-offset-1');
        $(".tagdiv").removeClass('col-xs-2').addClass('col-xs-4');
        $('#task-header').fadeOut(0, function() {
            $(this).text("Today").fadeIn(0);
        });
        is_next_seven = false;
    }
    ...
    $('#today_btn').click(function() {
        if (is_next_seven) {
            ...
            console.log(url);
            ajaxCall(url, { view: "today" }, 'PATCH');
        }
    });

    // /users/:id/update_settings(.:format)
    $('#next_seven_btn').click(function() {
        if (!is_next_seven) {
            ...
            setTimeout(function() {
                ...
                $('.tomorrow').fadeIn(600);
                $('.today').removeClass('col-xs-12').addClass('col-xs-3');
                $(".tagdiv").removeClass('col-xs-4').addClass('col-xs-2');
            }, 200);
            // $('#task-header').fadeOut(200, function() {
            //     $(this).text("Next Seven Day").fadeIn(300);
            // });
            is_next_seven = true;
            url = "/users/" + $(this).attr('data-user') + "/update_settings";
            console.log(url);
            ajaxCall(url, { view: "seven" }, 'PATCH');
        }
    });
    ...
    $('.toggle_completed').click(function() {
        ...
        if (text.indexOf("Hide") == 0) {
            ...
            url = "/users/" + $(this).attr('data-user') + "/update_settings"
            ajaxCall(url, { completed_hidden: "true" }, 'PATCH');
        } else {
            ...
            url = "/users/" + $(this).attr('data-user') + "/update_settings"
            ajaxCall(url, { completed_hidden: "false" }, 'PATCH');
        }
    });
...
function tagCheckboxClicked(dom) {
    dom.click(function() {
        var url = "/users/" + $(this).attr('data-userid') + "/update_settings";
        console.log(url);
...
var url;
var counter = 0;
var is_next_seven = true;

/* html snippets */
// var smsNeeded = '<span class="smsdesc"> Please enter your phone number to receive reminders through sms </span>';
// smsNeeded += '<%= form_for(@user, url: number_user_path(@user), html: { class: "form-inline smsformstyle", role: "form" }, remote: true) do |f| %>';
// smsNeeded += '<%= render "shared/error_messages" %>';
// smsNeeded += '<div class="form-group"> <%= f.text_field :number %> </div>';
// smsNeeded += '<button class="save-sms btn-xs btn-default" name="commit" type="submit"> Save </button> <% end %>';
// smsNeeded += '<span class="x-button ib"> </span> <div> <p class="updated hidden"> Updated! </p> </div> </span>';
...
$(document).ready(function() {
    ...
    // same as above except for exist
    // $('.exclamation-exist').click(function() {
    ...
    // show updated on save-sms notice
    //  $('.save-sms').click(function() {
    ...
    $("#Monday, #Tuesday, #Wednesday, #Thursday, #Friday, #Saturday, #Sunday").sortable({
        ...
        cursor: "move",
        distance: 4,
        // recieve call happens on list that recieves task if it is a new div
        receive: function(event, ui) {
            ...
            // getting url to update ordering
...
function completeClicked(dom) {
    dom.click(function() {
        if ($(this).hasClass("complete")) {
            ...
            $(this).siblings(".task_text").css('text-decoration', '');
            ...
            $(this).siblings(".task_text").css('text-decoration', 'line-through');
            console.log($(this).closest(".tasks").attr('data-url'));
            var url = $(this).closest(".tasks").attr('data-url') + "/complete"
            console.log(url);
            ajaxCall(url, {
