...
function checkPosition() {
    ...
    if ($('#desktopTest3').is(':hidden')) {
        // changeTextBtn("Today", "Hide", "Next Seven", "Show");
...
$(document).ready(function() {
	...
	if ($('#recieve_email').parent().attr('data-dailyemail') == "true") {
...
function dailyEmailCheckboxToggle(dom) {
	console.log($('#recieve_email').parent().attr('data-dailyemail'));
    dom.click(function() {
        var url = "/users/" + $(this).parent().attr('data-userid') + "/update_settings";
        console.log(url);
